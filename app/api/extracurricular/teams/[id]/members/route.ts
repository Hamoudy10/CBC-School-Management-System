...
import { listTeamMembers, addTeamMember, addTeamMemberSchema } from '@/features/extracurricular';

export const GET = withPermission('academics', 'view', async (_request, { user, params }) => {
  try { return successResponse(await listTeamMembers(params.id)); }
...
export const POST = withPermission('academics', 'create', async (request, { user }) => {
  const v = await validateBody(request, addTeamMemberSchema);
  if (!v.success) {return validationErrorResponse(v.errors!);}
  try { return successResponse(await addTeamMember(v.data!)); }
