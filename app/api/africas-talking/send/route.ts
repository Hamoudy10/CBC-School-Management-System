...
import { parentNotificationSchema, sendParentNotification } from '@/features/africas-talking';
...
export const POST = withPermission('communication', 'create', async (request: NextRequest, { user }) => {
  ...
  const validation = await validateBody(request, parentNotificationSchema);
  if (!validation.success) { return validationErrorResponse(validation.errors!); }

  try {
    const result = await sendParentNotification(validation.data!);
